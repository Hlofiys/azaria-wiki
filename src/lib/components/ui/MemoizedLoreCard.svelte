<!-- Memoized LoreCard with optimized re-rendering -->
<script lang="ts">
	import {
		Icon,
		getCategoryIcon,
	...
	let colors = $derived.by(() => getCategoryColors(entry.category));
	let categoryIcon = $derived.by(() => getCategoryIcon(entry.category));
	let categoryName = $derived.by(() => getCategoryNameRussian(entry.category));
	let entryUrl = $derived.by(() => resolve(`/${entry.category}/${entry.slug}` as `/${string}/${string}`));
	
	// Memoize limited tags to prevent recalculation
	let displayTags = $derived.by(() => entry.tags?.slice(0, 3) || []);
	let remainingTagsCount = $derived.by(() => Math.max(0, (entry.tags?.length || 0) - 3));
</script>
